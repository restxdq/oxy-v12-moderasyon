...
module.exports.run = async (client, message, args) => {
  ...
  //=================DATA BASE=================//
  db.push(`ceza_${oxy.id}`, `\n\`>\` <@!${message.author.id}> adlı kullanıcı tarafından \`${oxytarih}\` Tarihinde \`${sebep}\` Sebebiyle **Mutesi Kaldırıldı.**`);
  db.add(`cezaNo_${message.guild}`, 1);
  db.delete(`muted_${oxy.id}`);
  //=================DATA BASE=================//

message.channel.send(oxyemb.setFooter(`Ceza No : \`${cezaNo}\``).setColor(config.renk_yesil).setDescription(`<@!${message.author.id}> tarafından ${oxy} **${sebep}** **Mutesi Kaldırıldı.**`));
channels.muteLog.send(oxyemb.setFooter(`Ceza No : \`${cezaNo}\``).setColor(config.renk_siyah).setDescription(`\`•\` **Mutesi Açıldı.**\n\n**•** Mute Açan Yetkili : <@!${message.author.id}> (\`${message.author.id}\`)\n**•** Susturulan : <@!${oxy.id}> (\`${oxy.id}\`)\n**•** Mute Açılma Nedeni : ${sebep}\n**•** Tarih : ${oxytarih}`))
