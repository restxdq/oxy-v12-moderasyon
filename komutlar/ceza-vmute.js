...
module.exports.run = async (client, message, args) => {
  ...
  //=================DATA BASE=================//
  db.push(`ceza_${oxy.id}`, `\n\`>\` <@!${message.author.id}> adlı kullanıcı tarafından \`${oxytarih}\` Tarihinde \`${sebep}\` Sebebiyle **Seste Mutelendi.**`);
  db.add(`cezaNo_${message.guild}`, 1);
  db.add(`sürevMute_${oxy.id}`, zamandilimi);  
  db.add(`vmuted_${oxy.id}`, 1);
  //=================DATA BASE=================//

message.channel.send(oxyemb.setFooter(`Ceza No : \`${cezaNo}\``).setColor(config.renk_yesil).setDescription(`<@!${message.author.id}> tarafından ${oxy} **${sebep}** sebebiyle **${zamandilimi} boyunca** seste mute atıldı`));
{channels.muteLog.send(oxyemb.setFooter(`Ceza No : \`${cezaNo}\``).setColor(config.renk_siyah).setDescription(`\`•\` **Seste Mute Atıldı.**\n\n**•** Susturan Yetkili : <@!${message.author.id}> (\`${message.author.id}\`)\n**•** Susturulan : <@!${oxy.id}> (\`${oxy.id}\`)\n**•** Mutelenme Süresi : ${zamandilimi}\n**•** Tarih : ${oxytarih}`))
oxy.voice.setMute(true);}setTimeout(async function() {
channels.muteLog.send(oxyemb.setFooter(`Ceza No : \`${cezaNo}\``).setColor(config.renk_yesil).setDescription(`\`•\` **Ses Mute Süresi Doldu.**\n\n**•** Susturan Yetkili : <@!${message.author.id}> (\`${message.author.id}\`)\n**•** Susturulan : <@!${oxy.id}> (\`${oxy.id}\`)\n**•** Tarih : ${oxytarih}`))
